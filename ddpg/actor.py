...
class Actor():
    ...
    def __init__(self, 
        ...
        self.weights = [v for v in tf.compat.v1.trainable_variables() if 'actor' in v.op.name]
        ...
        self.target_weights = [v for v in tf.compat.v1.trainable_variables() if 'actor' in v.op.name][len(self.weights):]
        ...
        self.action_derivs = tf.compat.v1.placeholder(tf.float32, shape=(None, self.action_space_size))
        self.policy_gradient = tf.gradients(ys=self.infer, xs=self.weights, grad_ys=-self.action_derivs)
        #self.train = optimizer(learning_rate).apply_gradients(zip(self.policy_gradient, self.weights))
        self.train = optimizer.apply_gradients(zip(self.policy_gradient, self.weights))
