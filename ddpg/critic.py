...
class Critic():
    ...
    def __init__(self, sess, action_space_size, env_space_size, learning_rate=0.001, gamma=0.99, tau=0.001,
        ...
        self.tau = tau
        self.loss_val = 0
        self.state_ph = tf.compat.v1.placeholder(tf.float32, shape=(None, env_space_size), name='state_ph')
        self.action_ph = tf.compat.v1.placeholder(tf.float32, shape=(None, action_space_size), name='action_ph')
        ...
        self.weights = [v for v in tf.compat.v1.trainable_variables() if 'critic' in v.op.name]
        ...
        self.target_weights = [v for v in tf.compat.v1.trainable_variables() if 'critic' in v.op.name][len(self.weights):]
        ...
        self.loss = tf.reduce_mean(input_tensor=tf.square(self.wanted_qs - self.infer))

        self.train = optimizer.minimize(self.loss)

        self.gradient = tf.gradients(ys=self.infer, xs=self.action_ph) # Why gradients here?. Called in update
